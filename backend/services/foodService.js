...
module.exports={
   async createFood(req,restaurant){
    try{
      const food=new Food({anyfoodCategory:req.category,
...
async getRestaurantsFood(
    ...
    if(foodCategory) query.category=foodCategory;

    const foods=await Food.find(query).populate([{
        path:"ingredients",populate:{path:"category",select:"name"}},"foodCategory",
        {
...
async updateAvailabilityStatus(foodId){
    try{
        const food=await Food.findById(foodId).poopulate([
            {
